...
const initialNotifications = [
  { title: "Citation coverage healthy", detail: "96% cited answer coverage across 4 spaces", level: "Ready" },
  { title: "Retention review pending", detail: "Vendor Contract Template review due soon", level: "Watch" },
  { title: "Role sync available", detail: "Admin roles can be refreshed from workspace policy", level: "Action" },
];
...
export function App() {
  ...
  function viewNotification(title: string) {
                ...
                <div className="top-popover notification-popover">
                  ...
                  {notifications.length > 0 ? (
                    notifications.map((item) => (
                      <button key={item.title} type="button" onClick={() => viewNotification(item.title)}>
                        <span>{item.title}</span>
                        <small>{item.level} · {item.detail}</small>
                      </button>
                    ))
                  ) : (
